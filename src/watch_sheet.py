...
- 개발환경/빌드/설정(예: 프로젝트 생성, 빌드 성공 여부, 환경변수·설정값 지정)에 관한 요구사항은 TC 개수에 포함하지 말 것 — 이런 항목은 CLI 실행 결과·설정값·빌드 성공 여부로 검증하는 별도 계층이라 UI 기반 TC 대상이 아님
- 같은 실패 원인과 같은 기대결과를 갖는 시나리오를 기능/예외처리/회귀 등 테스트 유형만 바꿔서 중복 배정하지 말 것 — 유형이 다르더라도 실질적으로 같은 케이스면 하나의 유형에만 배정할 것
