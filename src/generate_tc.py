...
- 요구사항에 명시되지 않은 오류 메시지·오류 화면·fallback 동작을 기대결과에 임의로 만들어내지 말 것 — 요구사항에 문구/화면이 실제로 적혀있을 때만 그대로 인용하고, 없으면 "정책에 따라 처리됨" 수준으로만 작성할 것
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "요구사항에 정의되지 않은 오류 메시지·오류 화면·fallback 동작을 상상해서 채우지 마세요. "
                            "한국어로만 작성하세요."
                        ),
                    },
...
- 개발환경/빌드/설정(예: 프로젝트 생성, 빌드 성공 여부, 환경변수·설정값 지정)에 관한 요구사항은 TC 개수에 포함하지 마세요 — 이런 항목은 CLI 실행 결과·설정값·빌드 성공 여부로 검증하는 별도 계층이라 UI 기반 TC 대상이 아닙니다
- 같은 실패 원인과 같은 기대결과를 갖는 시나리오를 기능/예외처리/회귀 등 테스트 유형만 바꿔서 중복 배정하지 마세요 — 유형이 다르더라도 실질적으로 같은 케이스면 하나의 유형에만 배정하세요
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "개발환경/빌드/설정 요구사항은 TC 대상이 아니므로 배정에서 제외하세요. "
                            "동일한 실패 원인과 기대결과를 유형만 바꿔 중복 배정하지 마세요. "
                            "JSON만 출력하세요."
                        ),
                    },
                    {"role": "user", "content": prompt},
