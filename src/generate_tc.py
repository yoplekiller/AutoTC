...
[QA 관점의 테스트케이스 생성 원칙]
TC는 구현 완료된 결과물이 요구사항을 만족하는지 검증하는 관점으로 작성하고, 구현 방법(어떻게 만드는지)을 설명하지 않는다.

환경 구축/SDK 설정/Build/CI/Test Framework/설정 파일 관련 요구사항에는 다음을 적용한다:
1. 설정 파일을 생성하거나 수정하는 절차를 테스트 단계로 쓰지 않는다.
   잘못된 예: "설정 파일을 연다." / "값을 입력한다." / "설정 파일을 저장한다."
   올바른 예: "설정 파일이 존재하는지 확인한다." / "요구된 설정값이 적용되어 있는지 확인한다." / "관련 명령을 실행하여 정상 동작 여부를 확인한다."
2. 개발 작업을 테스트 절차로 변환하지 않는다.
   잘못된 예: "Vitest 환경을 구성한다."
   올바른 예: "Vitest 테스트 명령을 실행하고 정상적으로 실행되는지 확인한다."

그 외 모든 요구사항에 공통 적용:
3. 요구사항에 정의되지 않은 오류 메시지·오류 화면·오류 로그·경고 팝업·fallback 동작·자동 복구 동작을 기대결과로 임의로 생성하지 않는다. 명시되지 않았다면 "요구사항 미충족으로 판정됨"처럼 검증 결과로 표현한다.
4. 정상 요구사항을 단순히 반대로 뒤집어서 Negative TC를 생성하지 않는다.
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "환경/설정/빌드/CI 관련 요구사항은 설정 파일을 만드는 절차가 아니라, 설정값·명령 실행 결과를 확인하는 절차로 쓰세요. "
