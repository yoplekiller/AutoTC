...
### TC Quality Gate (2026-08-24)
기획서만 보고 TC를 뽑으면 요구사항에 근거 없는 케이스, 수동 QA로 바로 실행 불가능한 케이스,
기대결과가 "정상적으로 처리됨" 수준에 머무는 케이스가 섞여 나오는 문제가 있었습니다.
AutoTC를 **AI가 QA를 대신하는 도구가 아니라, QA가 검토할 후보 TC를 사전 정제해주는
Test Design Assistant**로 보고 아래 게이트를 추가했습니다.

- **요구사항 추적**: 티켓/기획서에서 추론한 요구사항에 `REQ-N` 번호를 매기고, 각 TC가 어떤
  REQ를 근거로 삼았는지 명시. 근거가 없으면 `requirement_status: NEEDS_CLARIFICATION`으로
  표시하고 임의로 확정 TC를 만들지 않음
- **실행 가능성 분류**: 수동으로 바로 실행 불가능한 TC(예: 서버 500 강제 응답, Mock 필요)를
  삭제하지 않고 `execution_type`(MANUAL / MANUAL_WITH_TOOL / API_OR_MOCK_REQUIRED /
  AUTOMATION_RECOMMENDED / ENVIRONMENT_SETUP_REQUIRED / NOT_EXECUTABLE /
  REQUIREMENT_CLARIFICATION)으로 분류만 함
- **PASS / REVIEW / REJECT 판정**: 요구사항 근거·실행 가능성·관찰 가능성·중복 여부를
  명시적 기준으로 AI가 자체 판정(`quality_status` + `quality_reason`). REJECT도 결과에서
  지우지 않고 색상으로 구분해 QA가 최종 판단하도록 남겨둠
- **LLM 자기평가 이중 체크**: LLM이 근거 없이 PASS를 자칭하거나("요구사항 근거 없음"인데
...
| TC ID | 대분류 | 소분류 | 테스트유형 | 테스트 시나리오 | 사전 조건 | 테스트 단계 | 기대 결과 | 테스트 상태 | 비고/버그 링크 | 우선순위 | 위험도 | Quality 판정 | 판정 사유 |
|-------|--------|--------|-----------|--------------|---------|------------|---------|-----------|-------------|---------|-------|-------------|---------|

**숨김 컬럼** (자동화/근거 확인용 — 필요할 때만 펼쳐서 확인)

| 자동화 가능여부 | 요구사항 근거 | 요구사항 상태 | 실행 가능성 | 필요 도구 | 실행 메모 | 설계 기법 |
|---------------|-------------|-------------|-----------|---------|---------|---------|
...
| 저품질 후보 TC 걸러내기 | QA가 전수 검토 | AI가 요구사항 근거/실행가능성/관찰가능성 기준으로 PASS·REVIEW·REJECT 1차 판정 (삭제 아님) |
